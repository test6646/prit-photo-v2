...
export default function Navbar() {
  ...
  // Fixed hover transition with proper type
    ...
    <>
      ...
      <nav
        className="
          ...
          backdrop-blur-lg bg-[var(--background)]/80
          py-3 border-b border-[var(--border)]
        "
      >
        <div className="flex items-center justify-between px-5 sm:px-7 md:px-10 h-[64px]">
          ...
          <div
            className="
              hidden md:flex items-center gap-8 lg:gap-7
              text-[13px] tracking-[1.4px] [font-family:blauer]
            "
          >
            {menuItems.map((item) => (
              <motion.button
          ...
          <Link
            href="/admin"
            className="hidden md:block hover:text-[var(--silver)] transition-colors border border-[var(--silver)]/50 p-3 rounded-full text-sm"
          >
            <svg
              fill="currentColor"
              width="25px"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="
              fixed inset-0 z-40 bg-[var(--background)]/95 backdrop-blur-2xl
              flex flex-col items-center justify-center
            "
          >
            <ul className="flex flex-col items-center gap-3 text-3xl uppercase tracking-widest [font-family:blauer]">
              {menuItems.map((item) => (
                <li key={item.id}>
                  <button
                    onClick={() => scrollToSection(item.id)}
              ...
              <li>
                <Link
                  href="/admin"
                  className="md:hidden block hover:text-[var(--silver)] transition-colors border border-[var(--silver)]/50 p-3 rounded-full text-sm mt-6"
                >
                  <svg
                    fill="currentColor"
                    width="25px"
